...
def update_position(self):
    ticker = ""
    if self.type == StrategyType.DCA:
        ticker = self.stock
    elif self.type == StrategyType.GRID:
        ticker = self.stock + "-" + StrategyType.GRID.name
    ...
    cursor.execute('''INSERT OR REPLACE INTO positions (stock, buy_count, last_buy_price, volume, total_cost, avg_price, current_price, profit, status, last_updated)
                          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                   (ticker, self.buy_count, self.last_buy_price, self.volume, self.total_cost, self.avg_price,
