...
class GridStockTrader(StockTrader):
    def __init__(self, index, stock, logger):
        ...
        super().__init__(f"{self.ticker}-Grid{self.index}", logger)
    ...
    def buy_stock(self, xch_volume, xch_price, stock_price):
        ...
        buy_price = buy_price if buy_price < stock_price else stock_price
        volume = xch_volume / (buy_price / xch_price)
        ...
        record_trade(self.stock, "BUY", buy_price, volume, xch_volume, 0)
        self.logger.info(f"Buying {volume} shares of {self.stock} at ${buy_price}")
    ...
    def sell_stock(self, xch_price, stock_price, liquid=False):
        ...
        sell_price = sell_price if sell_price > stock_price else stock_price
        if liquid:
            sell_price = stock_price
        request_xch = self.volume * (sell_price / xch_price)
        timestamp = datetime.now()
        if not send_asset(STOCKS[self.stock]["sell_addr"], self.wallet_id, request_xch,
                          self.volume, self.logger):
            # Failed to send order
            return
        record_trade(self.stock, "SELL", sell_price, self.volume, self.total_cost, request_xch - self.total_cost)
        self.logger.info(
            f"Selling {self.volume} shares of {self.stock} at ${sell_price} with {self.profit} XCH profit")
    ...
    while True:
        ...
        for trader in traders:
            if trader.ticker not in stocks_stats:
                stocks_stats[trader.ticker] = {"buying": 0, "selling": 0, "position": 0, "volume": 0, "arbitrage": 0, "profit": 0}
            stock_price = float(get_stock_price_from_dinari(trader.ticker, logger)[0])
            if stock_price == 0:
            ...
            stocks_stats[trader.ticker]["arbitrage"] += trader.buy_count if trader.position_status != PositionStatus.PENDING_BUY.name else trader.buy_count - 1
            ...
            if trader.position_status == PositionStatus.TRADABLE.name:
                if trader.max_price / CONFIG["XCH_MIN"] - trader.index * trader.grid_width >= stock_price / xch_price and trader.volume == 0:
                    trader.buy_stock(trader.invested_xch / trader.grid_num, xch_price, stock_price)
                if trader.max_price / CONFIG["XCH_MIN"] - (trader.index+1) * trader.grid_width < stock_price / xch_price and trader.volume > 0:
                    trader.sell_stock(xch_price, stock_price)
            else:
