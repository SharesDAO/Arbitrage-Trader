...
class DCAStockTrader(StockTrader):
    ...
    def buy_stock(self, crypto_volume, crypto_price, stock_price):
        ...
        customer_id = send_asset(STOCKS[self.stock]["buy_addr"], 1, self.ticker, volume, crypto_volume, self.logger, self.stock + "-DCA")
        if not customer_id:
        ...
        record_trade(self.stock, "BUY", stock_price, volume, crypto_volume, 0, customer_id)
    ...
    def sell_stock(self, crypto_price, stock_price, liquid=False):
        ...
        if self.profit >= CONFIG["MIN_PROFIT"] or liquid:
            ...
            if order_value_usd < 5.0:
                ...
                return
            timestamp = datetime.now()
            customer_id = send_asset(STOCKS[self.stock]["sell_addr"], 0, self.ticker, request_crypto,
