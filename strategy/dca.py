...
class DCAStockTrader(StockTrader):
    ...
    def handle_price_drop(self, xch_price, stock_price):
        self.current_price = stock_price
        self.profit = self.volume * self.current_price / xch_price / self.total_cost - 1
        ...
        if drop_percentage >= CONFIG["DCA_PERCENTAGE"] and self.buy_count < CONFIG["MAX_BUY_TIMES"]:  # 5% drop
            ...
            self.buy_stock(CONFIG["BUY_PERCENTAGE"] * CONFIG["INVESTED_XCH"], xch_price, stock_price)  # Repurchase the same volume
...
def execute_dca(logger):
    ...
    while True:
        ...
        for trader in traders:
            current_price = float(get_stock_price_from_dinari(trader.stock, logger)[1])
            if current_price == 0:
            ...
            if trader.position_status == PositionStatus.TRADABLE.name and is_market_open(logger):
                if trader.volume == 0:
                    trader.buy_stock(CONFIG["BUY_PERCENTAGE"] * CONFIG["INVESTED_XCH"], xch_price, current_price)
                elif trader.volume > 0:
                    trader.sell_stock(xch_price, current_price)  # Try to sell if profit threshold is met
                    if trader.position_status == PositionStatus.TRADABLE.name:
                        trader.handle_price_drop(xch_price, current_price)  # Handle price drop and repurchase logic
            else:
